...
スクロールしない 1 画面。Google レコーダーのタイルと同じ並びで、上に小さなアプリアイコンと名前、
中央に **横長の大きな「● 録音」ピル**（NOTELOOP 本体と同じ紫グラデーション。録音中も紫のまま）、下に小さなピル。

| 状態 | 表示 | 操作 |
|---|---|---|
| 待機中 | 大きなピルに「● 録音」。下の小さなピルは「録音一覧」 | タップで録音開始 |
| 録音中 | 大きなピルの中に **音量で動くゲージバー（5 本）** と経過時間。上に「録音中」、下の小さなピルは操作の案内 | **タップで一時停止**、もう一度タップで再開、**2 秒長押しで停止**（長押し中はピルが左から白く満ちていく） |
| 一時停止中 | ゲージは最小、上に「一時停止中」（経過時間は止まる） | タップで再開、長押しで停止 |

- タイル（ウィジェット）も同じ並び。待機中は大きなピルで録音開始（アプリが開いて権限確認のうえ開始）、小さなピルで録音一覧、
  録音中は大きなピルに「録音中」と経過時間を表示しタップでアプリを開く
