...
function generateMinutes(transcript) {
  // 議事録づくりでも変換辞書を参照し、登録した表記に統一する
  const text = autoConvert((transcript || '').replace(/\s+/g, ' ').trim());
 ...
 *   正しい表記をセットで登録する辞書。文字起こし・議事録の作成・AI連携で
 *   自動的に参照され、設定画面と「用語の確認・修正」の両方から追加・変更できる。
 * =======================================================*/
...
/* ---- 設定画面の変換辞書（追加・変更・削除） ---- */
function showDictNote(msg) {
  if (dictNote) dictNote.textContent = msg;
}
...
function checkTerms() {
  ...
  if (dictAutoApply) { applyDictToTranscript(true); return; }
...
function buildClaudePrompt() {
  const m = currentMinutes();
  const transcript = autoConvert(liveTranscript.value.trim());
...
【会議情報】
会議名: ${m.name}
日付: ${formatDateJp(m.date)}${partLine}${dictPromptBlock()}${draft}
